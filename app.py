...
html,body,[class*="css"]{font-family:'DM Sans',sans-serif!important;background-color:#0f0f0f!important;color:#fff;}
...
[data-testid="stFileUploader"]{background:rgba(255,255,255,.04)!important;border:2px dashed rgba(126,217,87,.3)!important;border-radius:16px!important;padding:8px!important;}
[data-testid="stFileUploaderDropzone"]{background:transparent!important;}
.stTextInput>div>div{border-color:rgba(255,255,255,.1)!important;background:rgba(255,255,255,.05)!important;}
...
def card(content, bg="rgba(255,255,255,0.04)", border="rgba(255,255,255,0.08)"):
    h(f'<div style="background:{bg};border:1px solid {border};border-radius:16px;padding:16px 20px;margin-bottom:12px;">{content}</div>')

def slabel(text):
    h(f'<div style="color:rgba(255,255,255,0.4);font-size:11px;font-family:Space Mono,monospace;letter-spacing:2px;margin-bottom:10px;margin-top:4px;">{text}</div>')
...
def page_camera():
    h('<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;"><div><div style="color:#7ED957;font-family:Space Mono,monospace;font-size:11px;letter-spacing:2px;">SMART SORT</div><div style="font-size:24px;font-weight:700;color:#fff;">Classify Item</div></div><div style="font-size:28px;">♻️</div></div>')
    ...
    if uploaded:
        ...
        h('<div style="text-align:center;color:rgba(255,255,255,0.4);font-size:12px;margin-top:4px;margin-bottom:8px;">Image ready to classify</div>')
    else:
        h('<div style="background:linear-gradient(160deg,#0a120a,#111a0f);border:2px dashed rgba(126,217,87,0.25);border-radius:20px;height:200px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;margin-bottom:12px;"><div style="font-size:48px;opacity:0.6;">📷</div><div style="color:rgba(255,255,255,0.35);font-size:13px;">Upload a photo to classify</div></div>')
    ...
    if st.button("♻️  Classify Item", disabled=disabled, use_container_width=True, type="primary"):
        ...
        st.rerun()
    if disabled:
        h('<div style="text-align:center;color:rgba(255,255,255,0.3);font-size:12px;margin-top:6px;">Upload an image first</div>')
    ...
    for icon, title, desc in [
        ...
        card(f'<div style="display:flex;gap:16px;align-items:flex-start;"><div style="font-size:22px;">{icon}</div><div><div style="color:#fff;font-weight:600;font-size:14px;">{title}</div><div style="color:rgba(255,255,255,0.5);font-size:12px;margin-top:3px;">{desc}</div></div></div>')

def page_result():
    if st.session_state.result is None:
        h('<div style="text-align:center;padding:60px 20px;"><div style="font-size:52px;margin-bottom:16px;">📸</div><div style="color:rgba(255,255,255,0.4);font-size:14px;line-height:1.8;">No result yet.<br>Go to the Camera tab and classify an item first.</div></div>')
    ...
    h(f'<div style="margin-bottom:16px;"><div style="color:rgba(255,255,255,0.4);font-family:Space Mono,monospace;font-size:11px;letter-spacing:2px;margin-bottom:6px;">CLASSIFICATION RESULT</div><div style="font-size:24px;font-weight:700;color:#fff;">{r["item"]}</div><div style="color:rgba(255,255,255,0.4);font-size:13px;">{r["category"]}</div></div>')
    if st.session_state.uploaded_img:
        st.image(st.session_state.uploaded_img, use_container_width=True)
        st.markdown(" ")

    card(f'<div style="display:flex;align-items:center;gap:18px;"><div style="width:56px;height:56px;border-radius:28px;background:{vc};display:flex;align-items:center;justify-content:center;font-size:26px;flex-shrink:0;">{vi}</div><div><div style="color:{vc};font-size:20px;font-weight:700;margin-bottom:4px;">{vt}</div><div style="color:rgba(255,255,255,0.5);font-size:13px;">{r["category"]}</div></div></div>', bg=vbg, border=vbd)

    slabel("WHERE TO THROW IT")
    card(f'<div style="display:flex;align-items:center;gap:14px;"><div style="width:48px;height:48px;border-radius:12px;background:{bc};display:flex;align-items:center;justify-content:center;font-size:24px;flex-shrink:0;">{bi}</div><div><div style="color:#fff;font-weight:600;font-size:16px;">{r["bin"]} Bin</div><div style="color:{bc};font-size:12px;margin-top:2px;">Check your local bin colour</div></div></div>', border=f"{bc}40")

    slabel("💡 TIP")
    card(f'<div style="color:rgba(255,255,255,0.75);font-size:13px;line-height:1.7;">{r["tip"]}</div>')
...
def page_profile():
    ...
    sync = "<div style='color:rgba(255,255,255,0.4);font-size:12px;margin-top:2px;'>✅ Syncing across devices</div>" if logged_in else ""
    avatar_bg = "linear-gradient(135deg,#7ED957,#4CAF50)" if logged_in else "rgba(255,255,255,0.1)"
    avatar_ic = "👤" if logged_in else "👻"
    name      = "Gergely K." if logged_in else "Guest"
    h(f'<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;padding-bottom:16px;border-bottom:1px solid rgba(255,255,255,0.07);"><div><div style="color:rgba(255,255,255,0.4);font-family:Space Mono,monospace;font-size:11px;letter-spacing:2px;margin-bottom:4px;">PROFILE</div><div style="color:#fff;font-size:22px;font-weight:700;">{name}</div>{sync}</div><div style="width:52px;height:52px;border-radius:26px;font-size:24px;display:flex;align-items:center;justify-content:center;background:{avatar_bg};">{avatar_ic}</div></div>')
    ...
    slabel("THIS MONTH")
    c1,c2 = st.columns(2)
    with c1: card(f'<div style="color:#7ED957;font-size:30px;font-weight:700;font-family:Space Mono,monospace;">{pct}%</div><div style="color:rgba(255,255,255,0.5);font-size:11px;margin-top:4px;">Recycled correctly</div>', bg="rgba(126,217,87,0.1)", border="rgba(126,217,87,0.2)")
    with c2: card(f'<div style="color:#fff;font-size:30px;font-weight:700;font-family:Space Mono,monospace;">{total}</div><div style="color:rgba(255,255,255,0.5);font-size:11px;margin-top:4px;">Items scanned</div>')

    h(f'<div style="margin-bottom:16px;"><div style="display:flex;justify-content:space-between;margin-bottom:6px;"><span style="color:rgba(255,255,255,0.5);font-size:12px;">Recycling rate</span><span style="color:#7ED957;font-size:12px;font-family:Space Mono,monospace;">{rec_count}/{total}</span></div><div style="height:8px;background:rgba(255,255,255,0.08);border-radius:4px;"><div style="width:{pct}%;height:8px;background:linear-gradient(90deg,#7ED957,#4CAF50);border-radius:4px;"></div></div></div>')

    slabel("BY BIN")
    for bn, cnt in bin_counts.items():
        col = BIN_COLORS.get(bn,"#fff")
        bw  = int((cnt/total)*120) if total else 0
        h(f'<div style="display:flex;align-items:center;gap:10px;margin-bottom:8px;"><div style="width:10px;height:10px;border-radius:5px;background:{col};flex-shrink:0;"></div><div style="color:rgba(255,255,255,0.6);font-size:12px;flex:1;">{bn}</div><div style="color:#fff;font-size:12px;font-family:Space Mono,monospace;">{cnt}</div><div style="height:4px;width:{bw}px;background:{col};border-radius:2px;opacity:0.7;"></div></div>')

    st.markdown("---")
    slabel("RECENT SCANS")
    for entry in history[:10]:
        col  = BIN_COLORS.get(entry["bin"],"#fff")
        icon = "♻️" if entry["recyclable"] else "🚫"
        rbg  = "rgba(126,217,87,0.12)" if entry["recyclable"] else "rgba(255,80,80,0.12)"
        h(f'<div style="display:flex;align-items:center;gap:12px;padding:12px 0;border-bottom:1px solid rgba(255,255,255,0.05);"><div style="width:36px;height:36px;border-radius:10px;background:{rbg};display:flex;align-items:center;justify-content:center;font-size:16px;flex-shrink:0;">{icon}</div><div style="flex:1;min-width:0;"><div style="color:#fff;font-size:13px;font-weight:500;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{entry["item"]}</div><div style="color:rgba(255,255,255,0.35);font-size:11px;">{entry["date"]}</div></div><div style="padding:3px 10px;border-radius:6px;background:{col}20;border:1px solid {col}40;color:{col};font-size:10px;font-family:Space Mono,monospace;white-space:nowrap;">{entry["bin"]}</div></div>')

    st.markdown(" ")
    if logged_in:
        if st.button("Sign Out", use_container_width=True):
            st.session_state.logged_in=False; st.rerun()
        h('<div style="text-align:center;color:rgba(255,255,255,0.25);font-size:11px;margin-top:6px;">Signed in as gergely@student.buas.nl</div>')
    else:
        slabel("ACCOUNT")
        ...
        if st.button("Sign In / Create Account", use_container_width=True, type="primary"):
            if email and password:
                st.session_state.logged_in=True; st.success("Signed in!"); time.sleep(0.6); st.rerun()
            else:
